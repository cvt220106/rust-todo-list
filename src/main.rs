...
#[launch]
fn rocket() -> _ {
    rocket::build().mount("/", routes![hello, add_task, read_tasks, edit_task, delete_task])
}
...
fn edit_task(task_update: Json<TaskUpdate<'_>>) -> &'static str {
    ...
    for line in reader.lines() {
        ...
        if line_pieces[0].parse::<u8>().expect("unable parse id as u8") == task_update.id {
    ...
    std::fs::rename("temp.txt", "tasks.txt").expect("unable to renmae temp.txt to tasks.txt");
...
fn delete_task(task_id: Json<TaskId>) -> &'static str {
    ...
    for line in reader.lines() {
        ...
        if line_pieces[0].parse::<u8>().expect("unable parse id as u8") != task_id.id {
            let task = format!("{}\n", line_string);
            temp.write(task.as_bytes())
                .expect("could not write to temp file");
        }
